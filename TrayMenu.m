...
- (NSMenu *)createMenu {
  ...
  // Add About
  menuItem =
      [menu addItemWithTitle:@"About MiddleClick" action:@selector(openWebsite:) keyEquivalent:@""];
  [menuItem setTarget:self];

  /*	clickItem = [menu addItemWithTitle:@"3 Finger Click"
     action:@selector(setClick:) keyEquivalent:@""]; [clickItem setTarget:self];

          tapItem = [menu addItemWithTitle:@"3 Finger Tap"
     action:@selector(setTap:) keyEquivalent:@""]; [tapItem setTarget:self];
          [self setChecks];
